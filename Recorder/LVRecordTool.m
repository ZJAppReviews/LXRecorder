...
- (void)stopRecording {
    if ([_recorder isRecording]) {
        ...
        self.endTimeStamp = [NSString stringWithFormat:@"%.0f", [[NSDate date] timeIntervalSince1970] * 1000];
    }
}
...
- (AVAudioRecorder *)recorder {
    if (!_recorder) {
        ...
        _timeStamp = [NSString stringWithFormat:@"%.0f", [[NSDate date] timeIntervalSince1970] * 1000];
        NSString *filePath = [path stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.caf", _timeStamp]];
...
- (NSURL *)getVideoPath {
    return [NSURL URLWithString:[NSString stringWithFormat:@"%@.caf", _timeStamp]];
}
...
- (NSString *)getTimerBetween {
    NSInteger time = (self.endTimeStamp.longLongValue - self.timeStamp.longLongValue) / 1000;
